...
<div class="message-card @if($isSender) mc-sender @endif" data-id="{{ $id }}">
    ...
    @if ($isSender)
        <div class="actions">
            <i class="fas fa-trash delete-btn" data-id="{{ $id }}" title="Eliminar mensaje"></i>
        </div>
    @endif
    ...
    <div class="message-card-content">
        @if (@$attachment->type != 'image' || $message)
            <div class="message">
                ...
                <a href="{{ route(config('chatify.attachments.download_route_name'), ['fileName'=>$attachment->file]) }}" class="file-download" title="Descargar archivo adjunto">
        ...
        <div class="image-wrapper" style="text-align: {{$isSender ? 'end' : 'start'}}">
            <div class="image-file chat-image" style="background-image: url('{{ Chatify::getAttachmentUrl($attachment->file) }}')" title="Ver imagen adjunta">

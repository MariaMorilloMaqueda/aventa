                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="botones-misprendas">
                                        <!-- SI ES UN ADMINISTRADOR PUEDE CANCELAR O ACEPTAR UN INTERCAMBIO -->
                                        @if (Auth::user()->esAdmin())
                                            @if (strtolower($intercambio->estado) === 'pendiente')
                                                <form action="{{ route('aceptarintercambio', $intercambio->id) }}" method="post" onsubmit="return confirm('¿Quieres FORZAR la aceptación de este intercambio?');">
                                                    @csrf
                                                    <button type="submit" class="boton boton-icono" title="Forzar Aceptar">
                                                ...
                                                <form action="{{ route('cancelarintercambio', $intercambio->id) }}" method="post" onsubmit="return confirm('¿Estás seguro de que quieres cancelar este intercambio?');">
                                                    @csrf
                                                    <button type="submit" class="boton boton-rojo boton-icono" title="Forzar Cancelar">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                            ...
                                            @endif
                                        <!-- SI ES UN USUARIO REGISTRADO PUEDE: -->
                                        @else
                                            <!-- Si el intercambio está en estado completado y ninguno o solo un usuario ha valorado aparece el botón "valorar" -->
                                            ...
                                            <!-- Si el intercambio está en estado valorado o cancelado o al menos el usuario registrado ha añadido una valoración no aparece nada -->
                                            @elseif (in_array(strtolower($intercambio->estado), ['cancelado', 'valorado']))

                                            <!-- Si el intercambio está en estado pendiente aparece el botón "chatear" -->
                                            @else 
                                                ...
                                                @if (strtolower($intercambio->estado) === 'pendiente')
                                                    <!-- Si el intercambio está en estado pendiente y el usuario registrado es receptor aparece el botón "aceptar" -->
                                                    ...
                                                    <!-- Si el intercambio está en estado pendiente aparece el botón "cancelar" -->
                                                    <form action="{{ route('cancelarintercambio', $intercambio->id) }}" method="post" onsubmit="return confirm('¿Estás seguro de que quieres cancelar este intercambio?');">
                                                        @csrf
